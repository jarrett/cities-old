...
// may wrap a struct implementing the Widget trait. The inner Widget could
...
impl Element {
  ...
  // Reserves a slot in the UI's allocator. Buffers vertex data (size
  // and position) into that slot.
  fn buffer(&self, ui: &mut Ui) {
    let position_data: Vec<GLint> = vec![
    ...
    // Because each Element has its own Texture, the UV coords are
    // always just the four corners of the unit square. (This will need
    // to change if we ever optimize by combining UI texstures into
    // spritesheets.)
    ...
    // 6 indices per slot. (Each rectangle is two triangles.)
    ...
    ui.position_buffer.buffer_sub_data(
      // Offset into position buffer. 4 bytes per int x 2 ints per vertex x
      // 4 vertices per slot = 32 bytes per slot.
      self.slot * 32,
      // Size of position data. 32 bytes per slot.
      32,
      &position_data
    );

    ui.uv_buffer.buffer_sub_data(
      // Offset into position buffer. 4 bytes per float x 2 floats per vertex x
      // 4 vertices per slot = 32 bytes per slot.
      self.slot * 32,
      // Size of attribute data. 32 bytes per slot.
      32,
      &uv_data
    );
    ...
    ui.index_buffer.buffer_sub_data(
      ...
      // 12 bytes per slot.
      self.slot * 12,
      // Size of index data. 12 bytes per slot.
      12,
      &index_data
    );
  }
